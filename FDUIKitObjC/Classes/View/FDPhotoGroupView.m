...
- (BOOL)shouldClipToTop:(CGSize)imageSize forView:(UIView *)view {
    ...
    if (view.width < 1 || view.fd_height < 1) return NO;
    return imageSize.height / imageSize.width > view.width / view.fd_height;
}

- (id)copyWithZone:(NSZone *)zone {
    FDPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
@property (nonatomic, strong) FDPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation FDPhotoGroupCell
...
- (void)setItem:(FDPhotoGroupItem *)item {
    ...
    @weakify(self);
    if (![item.largeImageURL.absoluteString hasPrefix:@"http"]) {
        _imageView.image = [UIImage imageWithContentsOfFile:item.largeImageURL.absoluteString];
        self.maximumZoomScale = 3;
...
@interface FDPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation FDPhotoGroupView
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _pager.height = 10;
    _pager.center = CGPointMake(self.fd_width / 2, self.fd_height - 18);
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((FDPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    FDPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    FDPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    FDPhotoGroupCell *cell = [self cellForPage:currentPage];
    FDPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(FDPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            FDPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                FDPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (FDPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (FDPhotoGroupCell *)dequeueReusableCell {
    FDPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (FDPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (FDPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    FDPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
