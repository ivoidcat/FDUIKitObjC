...
#define SCROLL_WIDTH self.width
#define SCROLL_HEIGHT (self.height - _bottomLineHeight)

#define CURRENT_WIDTH(s) [titleWidthArray[s] floatValue] - 10
...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        if (self.tag == 1101) {
            button.frame = CGRectMake(start_x, 0, width, self.height);
            start_x += width;
        }else{
            button.frame = CGRectMake(start_x, 0, width, _slideHeight);
            start_x += (width + 15);
        }
        if (titles) {
            ...
            if (self.btnBgImg) {
                [button setBackgroundImage:[UIImage imageNamed:self.btnBeforeBgImg] forState:UIControlStateNormal];
            }
            //
...
//    }
    ...
    if (self.btnBgImg) {
        [select_btn setBackgroundImage:[UIImage imageNamed:self.btnBgImg] forState:UIControlStateNormal];
        [before_btn setBackgroundImage:[UIImage imageNamed:self.btnBeforeBgImg] forState:UIControlStateNormal];
        
    }else{
        [before_btn setBackgroundImage:nil forState:UIControlStateNormal];
    }
}
